  ...
  var Wizard = function (_React$Component) {
    ...
    _createClass(Wizard, [{
      ...
      value: function _getSteps() {
        ...
        return this.wizardItems.map(function (item) {
          return {
            id: item.props.id,
            index: _this2._getIndexById(item.props.id),
            title: item.props.title,
            cleared: _this2.state.cleared.find(function (it) {
              return it === item.props.id;
            }) ? true : false,
            active: _this2._isActive(item.props.id)
          };
        });
      }
    }, {
